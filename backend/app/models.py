...
class Control(Base):
    ...
    current_state: Mapped["ControlCurrentState | None"] = relationship(back_populates="control", uselist=False)
...
class ControlRun(Base):
    ...
    run_metadata_json: Mapped[dict | None] = mapped_column(JSONB)
    created_at: Mapped[datetime] = mapped_column(default=utcnow, nullable=False)
...
class ControlCurrentState(Base):
    ...
    last_run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("control_runs.id"))
    ...
    last_run: Mapped["ControlRun | None"] = relationship(foreign_keys=[last_run_id])
